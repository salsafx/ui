...
import { fxManagedIconAttr } from '../base/FxCore';
...
function normalizeCustomSize(size: string): string | null {
    ...
    if (!raw || sizeTokens.has(raw)) {
        ...
        return `${raw}px`;
    } else if (/^\d+(\.\d+)?(px|rem|em)$/i.test(raw)) {
        return raw;
    } else {
        return null;
    }
    ...
    private syncIcon() {
        ...
        if (this.hasManualContent || !value) {
            ...
            if (managed instanceof HTMLImageElement) {
                ...
                imgEl.setAttribute(fxManagedIconAttr, 'icon');
            ...
            iEl.setAttribute(fxManagedIconAttr, 'icon');
