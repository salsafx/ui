...
import { El, SlottedIconManager, Css } from '../base/FxCore';
...
export class FxIcon extends FxElement {
    ...
    private get hasManualContent(): boolean {
        return Array.from(this.children).some(
            (child) => child instanceof HTMLElement && !child.hasAttribute(SlottedIconManager.managedAttr),
        );
    }

    private manageIcon() {
        const managed = this.querySelector(
            `:scope > [${SlottedIconManager.managedAttr}]`,
        ) as HTMLElement | null;
        ...
        if (this.hasManualContent || !value) {
            ...
            if (managed instanceof HTMLImageElement) {
                ...
                this.appendChild(El.img({
                    [SlottedIconManager.managedAttr]: 'icon',
                    src: value,
                    alt: '',
                    'aria-hidden': 'true',
                }));
