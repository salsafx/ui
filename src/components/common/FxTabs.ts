...
import { SlottedIconManager, Variant } from '../base/FxCore';
...
export class FxTabs extends Animatable(FxContentElement) {
    ...
    private refreshTabs() {
        ...
        this.manageTabIcons(tabs);
        const signature = tabs
            .map((tab, index) => {
                const hasIcon = !!this.querySelector(
                    `:scope > [slot="${tabIconSlotName(index)}"]`,
                );
                return `${index}:${tab.header}:${tab.selectionColor}:${tab.variant}:${tab.badge}:${tab.icon}:${tab.disabled}:${hasIcon ? '1' : '0'}`;
            })
            .join('|');
    ...
    private manageTabIcons(tabs: FxTab[]) {
                ...
                tabs.forEach((tab, index) => {
                    ...
                    const manualOnTab = [...tab.children].filter(
                        (child): child is HTMLElement =>
                            ...
                            child.slot === 'icon' &&
                            !child.hasAttribute(SlottedIconManager.managedAttr),
                    );
    
                    if (manualOnTab.length > 0) {
                        this.slottedIcons.set(stateId, '', slotName);
                        ...
                        this.querySelectorAll(
                            `:scope > [slot="${slotName}"]:not([${SlottedIconManager.managedAttr}])`,
                        ).forEach((node) => node.remove());
                        this.slottedIcons.set(stateId, tab.icon, slotName);
                    } else {
    ...
    private resolveTabSelectionColor(tab: FxTab): string {
        return Variant.resolvedColor(tab.selectionColor, tab.variant, this.selectionColor);
    }
