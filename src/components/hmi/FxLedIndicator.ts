...
import { Variant, VariantAttributeConverters, type VariantAttribute } from '../base/FxCore';
...
export class FxLedIndicator extends Animatable(FxElement) {
    ...
    private get resolvedColor(): string {
        return Variant.resolvedColor(this.color, this.variant, '#22c55e');
    }

    private get colorVars(): string {
        const hex = this.resolvedColor.replace('#', '');
        const red   = parseInt(hex.substring(0, 2), 16);
        const green = parseInt(hex.substring(2, 4), 16);
        const blue  = parseInt(hex.substring(4, 6), 16);
        if (Number.isNaN(red) || Number.isNaN(green) || Number.isNaN(blue)) {
            return `--led-color: ${this.resolvedColor}`;
        }
