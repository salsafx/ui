...
export class FxLinearGauge extends Animatable(FxMeasureElement) {
    ...
    private get geometry() {
        ...
        const endPad = sides.end ? FxLinearGauge.compactScaleBand : FxLinearGauge.compactMinPad;
    ...
    private get scaleOffsets() {
        const {
            isHorizontal,
            geometry: { insetStart, wellLength, shellLength, viewCross, trackCenter }
        } = this;
    ...
    private renderScale() {
        const {
            scaleOffsets: { orientation, start, end, viewBoxWidth, viewBoxHeight, trackCenter }
        } = this;
