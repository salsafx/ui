...
function findSectorAtAngle(
    ...
    let found: RotarySector | null = null;
...
export class FxRotarySelector extends FxElement {
    static styles = css`
        :host {
    ...
    private emitChange(): void {
        this.dispatchEvent(
            new CustomEvent<RotaryChangeEvent>("change", {
                detail: {
    ...
    private handleAngle(e: MouseEvent | TouchEvent): void {
        if (!this.disabled) {
            ...
            if (sector && sector.id !== this.selectedSector) {
