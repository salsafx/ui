...
import { SlottedIconManager, Css } from '../base/FxCore';
...
export class FxSwitch extends Animatable(FxElement) {
    ...
    theme?: FxThemeMode;
    @property({ type: String, attribute: 'foreground-color' }) foregroundColor = '#ffffff';
    ...
    private readonly slottedIcons = new SlottedIconManager(this);

    private resolveStateWidth(): string {
        return Css.normalizeLength(this.stateWidth, '72px');
    }

    protected get effectiveStates(): FxSwitchState[] {
        if (this.states.length > 0) {
            return this.states;
        } else if (this.slottedStates.length > 0) {
            return this.slottedStates;
        } else {
            return [
                { id: 'off', label: 'OFF', foregroundColor: '#6b7280', backgroundColor: '#f3f4f6' },
                { id: 'on', label: 'ON', foregroundColor: '#ffffff', backgroundColor: '#a855f7' },
            ];
        }
    }
    ...
    private managingIcons = false;
    private iconPresenceKey = '';

    private manageIcons() {
        if (!this.managingIcons) {
            this.managingIcons = true;
            try {
                if (this.states.length > 0) {
                    this.slottedIcons.manage(this.states);
                } else {
                    const stateElements = this.getAssignedStateElements();
                    if (stateElements.length > 0) {
                        this.slottedIcons.manage(stateElements);
                    } else {
                        this.slottedIcons.manage(this.effectiveStates);
                    }
                }
                const key = this.effectiveStates
                    .filter((s) => this.slottedIcons.has(SlottedIconManager.slotName(s.id)))
                    .map((s) => s.id)
                    .join('\0');
    ...
    protected collectSlottedStates() {
        ...
        const next = elements.map((s) => ({
            ...
            foregroundColor: s.foregroundColor || undefined,
            backgroundColor: s.backgroundColor || undefined,
            disabled: s.disabled || undefined,
        }));
        if (!SlottedIconManager.statesEqual(this.slottedStates, next)) {
    ...
    protected handleStateChange(id: string) {
        if (!this.disabled) {
            ...
            if (stateObj && !stateObj.disabled && this.activeId !== id) {
                this.activeId = id;
    ...
    render() {
        ...
        const activeTextColor = activeState?.foregroundColor || this.foregroundColor || '#ffffff';
        ...
        return html`
            ...
            <div class="switch-container" style="--fx-switch-state-width: ${stateWidth};">
                ...
                ${states.map((stateObj) => {
                    ...
                    const isStateDisabled = !!stateObj.disabled || this.disabled;
                    const styleMap = isActive ? `--active-text-color: ${activeTextColor};` : '';
                    const slotName = SlottedIconManager.slotName(stateObj.id);
                    const hasIcon = !!stateObj.icon?.trim() || this.slottedIcons.has(slotName);
                    const stateClass = [
                        'switch-button',
                        isActive ? 'active' : '',
                        stateObj.disabled && !this.disabled ? 'is-disabled' : '',
                    ].filter(Boolean).join(' ');
