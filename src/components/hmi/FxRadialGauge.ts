...
export class FxRadialGauge extends Animatable(GlassOverlay(FxMeasureElement)) {
    ...
    private valueToAngle(value: number): number {
        ...
        const t = span === 0 ? 0 : (value - this.min) / span;
        return this.startAngle + Math.min(Math.max(t, 0), 1) * this.arcLength;
    }
    ...
    render() {
        ...
        const capInsetDeg = this.regions.length > 0
            ? ((strokeWidth / 2) / radius) * (180 / Math.PI)
            : 0;
        const trackStart = startAngle + capInsetDeg;
        const trackEnd = endAngle - capInsetDeg;
        const trackArcLength = Math.max(trackEnd - trackStart, 0);

        const trackPath = this.describeArc(centerX, centerY, radius, trackStart, trackEnd);
        const totalLength = (2 * Math.PI * radius) * (trackArcLength / 360);
        const dashOffset = totalLength * (1 - progress);
        const glassRadius = this.hasTrack ? radius + strokeWidth / 2 + 1 : radius;
        const regionRadius = radius + strokeWidth / 2 + 2.2;

        const regionArcs = this.regions.map((region) => {
            ...
            const a0 = this.valueToAngle(from);
            const a1 = this.valueToAngle(to);
            if (a1 <= a0) return nothing;
            const path = this.describeArc(centerX, centerY, regionRadius, a0, a1);
            return svg`
                <path
                    class="region-arc"
                    d="${path}"
