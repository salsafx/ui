...
import { SlottedIconManager, Variant, VariantAttributeConverters, type VariantAttribute } from '../base/FxCore';
...
export class FxPushButton extends Animatable(FxElement) {
    ...
    @property({ reflect: true, converter: VariantAttributeConverters.optional })
    ...
    private manageIcons() {
        if (!this.managingIcons) {
            ...
            try {
                ...
                const key = this.slottedIcons.has('icon') ? 'icon' : '';
    ...
    protected updated(changed: Map<string | number | symbol, unknown>) {
        if (changed.has('icon')) this.manageIcons();
    }
