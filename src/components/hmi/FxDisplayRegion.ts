...
import { isNil, SlottedIconManager } from '../base/FxCore';
...
export class FxDisplayRegion extends FxElement {
    ...
    @property({ type: String, attribute: 'content-layout', reflect: true, converter: DisplayAttributeConverters.contentLayout })
    contentLayout: FxDisplayContentLayout = 'labeled';

    @property({ type: String, reflect: true, converter: DisplayAttributeConverters.align })
    ...
    @property({ type: String, reflect: true, converter: DisplayAttributeConverters.typography })
    ...
    protected updated(changed: PropertyValues) {
        ...
        if (changed.has('typography')) {
            DisplayTypography.load(this.typography);
        }
        if (changed.has('icon')) this.manageIcon();
    }
    ...
    private manageIcon() {
        this.slottedIcons.manage(this.icon);
    }
    ...
    private renderIcon() {
        if (this.icon || this.slottedIcons.has('icon')) {
        ...
        const customStyles = html`
            <style>
                :host {
                    ...
                    ${DisplaySize.toCssVars(this.size)}
                }
                .label {
