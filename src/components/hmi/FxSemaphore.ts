...
import { SlottedIconManager, Variant, type VariantAttribute } from '../base/FxCore';
        ...
        if (!this.managingIcons) {
            ...
            try {
                ...
                if (stateElements.length > 0) {
                    ...
                    this.slottedIcons.manage(
                        this.effectiveStates.map((s) => ({ id: s.value, icon: s.icon })),
                    );
                }
                const key = this.effectiveStates
                    .filter((s) => this.slottedIcons.has(SlottedIconManager.slotName(s.value)))
                    .map((s) => s.value)
                    .join('\0');
    ...
    render() {
        ...
        return html`
            ...
            <div class="shell" role="img" aria-label="${this.label || 'semaphore'}">
                ${states.map((stateObj) => {
                    const slotName = SlottedIconManager.slotName(stateObj.value);
