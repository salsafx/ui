...
import { componentPalettes } from './ComponentPalettes';
...
export class ThemeManager implements IConfigurator {
    ...
    applyConfig(config: FxSetupConfig): void {
        ...
        for (const key of Object.keys(componentPalettes) as FxThemeComponentKey[]) {
            const mode = config.theme.components[key];
            const palette = componentPalettes[key] as Partial<Record<FxThemeMode, ThemeTokens>>;
            const tokens = palette[mode] ?? palette[defaultComponents[key]];
            if (!isNil(tokens)) {
